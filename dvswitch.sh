...
function downloadAndValidate() {
    ${DEBUG} curl --fail -o "$MMDVM_DIR/$1" -s "https://raw.githubusercontent.com/krot4u/Public_scripts/master/$2"
    if (( $? != 0 )); then
...
function downloadDatabases() {
    if [ -d "${MMDVM_DIR}" ] && [ -d "${AB_DIR}" ]; then
        ...
        ${DEBUG} curl -s -N "https://raw.githubusercontent.com/krot4u/Public_scripts/master/DMRIds.dat" > "${MMDVM_DIR}/DMRIds.dat"
        ...
        ${DEBUG} curl -s -N "https://raw.githubusercontent.com/krot4u/Public_scripts/master/DMR_Hosts.txt" > "${MMDVM_DIR}/DMR_Hosts.txt"
        ${DEBUG} curl -s -N "https://raw.githubusercontent.com/krot4u/Public_scripts/master/DMR_Hosts.txt" > "${MMDVM_DIR}/XLXHosts.txt"
        ...
        #downloadAndValidate "XLXHosts.txt" "XLXHosts.txt" "495"
        #downloadAndValidate "APRS_Hosts.txt" "APRS_Hosts.txt" "noam.aprs2.net"

        #declare isValid=`grep 5973272 "${MMDVM_DIR}/DMRIds.dat"`
        #if [ -z "$isValid" ]; then
        ...
        #   isValid=`grep 5973272 "${MMDVM_DIR}/DMRIds.dat"`
        #   if [ -z "$isValid" ]; then
