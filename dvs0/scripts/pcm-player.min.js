...
function DVSwitchPlayer(t, i) {
  (this.ws = null),
    (this.socketURL = "wss://dvs0.qra-team.online:8081"),
...
(PCMPlayer.prototype.init = function (t) {
  (this.option = Object.assign(
    {},
    { encoding: "16bitInt", channels: 1, sampleRate: 8e3, flushingTime: 1e3 },
    t
  )),
  ...
  (PCMPlayer.prototype.feed = function (t) {
    ...
    this.isTypedArray(t) &&
      ...
      (i = new Float32Array(this.samples.length + t.length)).set(
        this.samples,
        0
      ),
  ...
  (PCMPlayer.prototype.destroy = function () {
    this.stop(), this.audioCtx.close(), (this.audioCtx = null);
  }),
  ...
  (DVSwitchPlayer.prototype.play = function () {
    ...
    (this.btn.style.backgroundColor = "#008000"),
      ...
      this.ws.addEventListener("message", function (t) {
          ...
          let sumSquares = 0.0;
          for (const amplitude of e) { sumSquares += amplitude * amplitude; }
          var volume = Math.sqrt(sumSquares / e.length);
          var demo = document.getElementById('demo');
          var voiceLevel = volume - 135;//  по моим наблюдениям меньше 140 почти не бывает, как и больше 180
          if (!window.maxvoice || window.maxvoice < voiceLevel) {

              window.maxvoice = voiceLevel;
              console.warn(maxvoice);
          }
          demo.setAttribute('data-val', Math.max(0, voiceLevel));
        if (8e3 == this.sampleRate) r.player.feed(e);
          ...
          for (i = 0; i < e.length; i += 2)
            for (s = 0; s < n; s++) (o[a++] = e[i]), (o[a++] = e[i + 1]);
          r.player.feed(o);
        }
      }),
      this.player.play(),
  ...
  (DVSwitchPlayer.prototype.stop = function () {
    this.player.stop(),
      this.isPlaying() &&
        ((this.ws.closing = !0), this.ws.close(), (this.ws = null)),
